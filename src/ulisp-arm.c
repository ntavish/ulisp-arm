/* uLisp ARM Version 2.5a - www.ulisp.com
   ...
   Tavish Naruka <tavishnaruka@gmail.com> Jan 2019
...
#define micros() k_uptime_get_32()*1000

// Constants

#define TRACEMAX 10 // Number of traced functions
enum type { ZERO=0, SYMBOL=2, NUMBER=4, STREAM=6, CHARACTER=8, FLOAT=10, STRING=12, PAIR=14 };  // STRING and PAIR must be last
enum token { UNUSED, BRA, KET, QUO, DOT };
enum stream 
{ 
  SERIALSTREAM, 
#if defined(i2cenabled)
  I2CSTREAM, 
#endif
#if defined(spienabled)
  SPISTREAM, 
#endif
#if defined(sdcardsupport)
  SDSTREAM 
...
#define SYMBOLTABLESIZE 4096             /* Bytes */
uint8_t _end;
...
static unsigned int Freespace = 0;
static object *Freelist;
static char *SymbolTop = SymbolTable;
static unsigned int TraceFn[TRACEMAX];
static unsigned int TraceDepth[TRACEMAX];
#if defined(i2cenabled)
static unsigned int I2CCount;
#endif

static object *GlobalEnv;
static object *GCStack = NULL;
static object *GlobalString;
static int GlobalStringIndex = 0;
static char BreakLevel = 0;
static char LastChar = 0;
static char LastPrint = 0;
static char PrintReadably = 1;
...
static inline void myfree (object *obj) {
  car(obj) = NULL;
  cdr(obj) = Freelist;
  Freelist = obj;
  Freespace++;
...
#endif
#if defined(spienabled)
  if (streamtype == SPISTREAM) gfun = spiread;
  else 
#endif
...
object *edit (object *fun) {
  while (1) {
    ...
    else if (c == 'r') fun = item_read(gserial);
    else if (c == '\n') { pfl(pserial); superprint(fun, 0, pserial); pln(pserial); }
    else if (c == 'c') fun = cons(item_read(gserial), fun);
...
#define ppspecials 14
const char ppspecial[ppspecials] PROGMEM = 
  { DOTIMES, DOLIST, IF, SETQ, TEE, LET, LETSTAR, LAMBDA, WHEN, UNLESS, 
#if defined(i2cenabled)
  WITHI2C, 
#endif
  WITHSERIAL, 
...
void testescape () {
  if (console_read() == '~') error(PSTR("Escape!"));
}
...
void printobject (object *form, pfun_t pfun){
  if (form == NULL) pfstring(PSTR("nil"), pfun);
    ...
    pfstring(PSTR("<"), pfun);
  #if defined(spienabled)
    if ((form->integer)>>8 == SPISTREAM) pfstring(PSTR("spi"), pfun); else
  #endif
  #if defined(i2cenabled)  
    if ((form->integer)>>8 == I2CSTREAM) pfstring(PSTR("i2c"), pfun); else
  #endif
  #if defined(sdcardsupport)
    if ((form->integer)>>8 == SDSTREAM) pfstring(PSTR("sd"), pfun); else
  #endif
...
void loadfromlibrary (object *env) {   
  GlobalStringIndex = 0;
  object *line = item_read(glibrary);
  while (line != NULL) {
    eval(line, env);
    line = item_read(glibrary);
  }
}
#endif
...
object *readrest (gfun_t gfun) {
  ...
  while (item != (object *)KET) {
    if (item == (object *)BRA) {
      ...
      item = cons(symbol(QUOTE), cons(item_read(gfun), NULL));
    } else if (item == (object *)DOT) {
      tail->cdr = item_read(gfun);
...
object *item_read (gfun_t gfun) {
  object *item = nextitem(gfun);
  if (item == (object *)KET) error(PSTR("Incomplete list"));
  if (item == (object *)BRA) return readrest(gfun);
  if (item == (object *)DOT) return item_read(gfun);
  if (item == (object *)QUO) return cons(symbol(QUOTE), cons(item_read(gfun), NULL)); 
  return item;
}
